...
U = zeros(10)
for i in range(10):
    D[i, i] = expand(simplify(powsimp(powdenest(expand(H[i, i])))))

print(latex(D), file = lat)
...
for k in range(4):
    ...
    for i in range(4):
        print("|", file = fout, end = "\t")
        for j in range(4):
            conjSubmatrices[k][i,j] = simplify(conjSubmatrices[k][i,j] / (q - 1/q)**2)
            print(f"{conjSubmatrices[k][i,j]}", file = fout, end = ",\t")
        print("|", file = fout)
print("\n")
...
for i in range(4):
    for j in range(4):
        print("i = ", i, "j = ", j, file = fout)
        print(simplify(conjSubmatrices[i] - gens[j]), file = fout)
