...
def generatorMatrix():
    ...
    L[9, 6] = (-q**(1 - n) + q**(n - 1))**2
    L[9, 12] = q**(-2*dely)*(q**(2*n) - q**(2*n - 2) + 2)
    #L[9, 12] = (q**(2*n) - q**(2*n - 2) + 2)
    L[9, 9] = - L[9, 3] - L[9, 6] - L[9, 12]
    L[11, 14] = q*(q**(1 - 2*n) + q**(2*n - 1))
    L[11, 11] = - L[11, 14]
    L[12, 3] = (-q**(1 - n) + q**(n - 1))**2*q**(-2)
    L[12, 6] = (1 + q**(-2*dely)) * (q**(-2 + 2*n) - q**(-4 + 2*n) + 2*q**(-2)) * q**(2*dely) / (1 + q**(2*dely))
    #L[12, 6] = (q**(-2 + 2*n) - q**(-4 + 2*n) + 2*q**(-2))
    L[12, 9] = (1 + q**(-2*dely)) * (q**(-2 + 2*n) - q**(-4 + 2*n) + 2*q**(-2)) * 1 / (1 + q**(2*dely))
...
fromIndex = [13, 10, 4, 7, 10, 9, 9, 14, 14, 15]
toIndex =   [1, 2, 3, 4, 6, 7, 8, 9, 10, 11]
for i in range(len(fromIndex)):
    D = D.elementary_row_op(op = "n<->m", row1 = toIndex[i] - 1, row2 = fromIndex[i] - 1)
    D = D.elementary_col_op(op = "n<->m", col1 = toIndex[i] - 1, col2 = fromIndex[i] - 1)
